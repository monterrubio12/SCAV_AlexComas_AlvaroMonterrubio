...
Additionally, we are required to create a graphical user interface (GUI) as the final work of our API. The GUI will provide functionality for selecting a video file, select an output directory, and choosing one of the four codecs (VP8, VP9, H.265, or AV1) to apply the conversion. Further details about the GUI design and implementation will be explained in the subsequent sections.
 ...
 The file `practice_2.py` contains the implementation of the new functions for each of the two endpoints tasked in this practice. Notice that since some of the methods use functions implemented in previos deliveries we have imported at the header of the file the necessary classes from the `second_seminar.py` file. Also inside the `Practice_2` directory we have the file `integration_tests.py` that again imports the clases of `second_seminar.py` and includes the integration tests using the Big Buck Bunny video implemented in previous deliveries plus a new integration test for the encoding ladder to validate the functionality of the exercises. The file `main.py` specifies the routes for the new endpoint and instantiates `integration_tests.py`, and also conects our API with the created GUI. We also include the `unit_tests.py` file, which contains teh unit tests implemented in previpus deliveries plus 5 new unit tests for this practice. 4 of the tests are to check each of the codec formats and the fifth one is to check the functionality of the encoding lader. Finally inside `Practice_2` directory, we have `gui.py` where our GUI is designed.

We have created a new directory in our project named `Practice_2`. Inside this directory, we have included all the necessary Python files to run our API, operate our GUI, and implement the required endpoints:

 - `practice_2.py`:
   This file contains the implementation of the new functions for the two endpoints specified in this practice. Some of the methods rely on functions implemented in previous        exercises, so we have imported the necessary classes from the second_seminar.py file at the top of this file.
